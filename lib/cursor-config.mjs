...
export function applyCursorConfig({ openAIBaseUrl, addUserAddedModels, removeUserAddedModels } = {}) {
  ...
  try {
    ...
    if (Array.isArray(addUserAddedModels) && addUserAddedModels.length) {
      // 1. Append to userAddedModels so the names exist as choices.
      const existing = Array.isArray(data.aiSettings.userAddedModels) ? data.aiSettings.userAddedModels : [];
      ...
      // 2. Mark each as enabled so they're checked in the picker, and make
      //    sure they aren't on the explicitly-disabled list.
      const enabledExisting = Array.isArray(data.aiSettings.modelOverrideEnabled)
        ? data.aiSettings.modelOverrideEnabled
        : [];
      const enabledMerged = [...enabledExisting];
      const newlyEnabled = [];
      for (const m of addUserAddedModels) {
        if (!enabledMerged.includes(m)) {
          enabledMerged.push(m);
          newlyEnabled.push(m);
        }
      }
      if (newlyEnabled.length) {
        data.aiSettings.modelOverrideEnabled = enabledMerged;
        changes.push(`enabled in picker: ${newlyEnabled.join(', ')}`);
      }
      const disabledExisting = Array.isArray(data.aiSettings.modelOverrideDisabled)
        ? data.aiSettings.modelOverrideDisabled
        : [];
      const filteredDisabled = disabledExisting.filter((m) => !addUserAddedModels.includes(m));
      if (filteredDisabled.length !== disabledExisting.length) {
        data.aiSettings.modelOverrideDisabled = filteredDisabled;
      }
    }
