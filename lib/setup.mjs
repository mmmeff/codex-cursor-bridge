  ...
  try {
    ...
    if (tunnelChoice) {
      stdout.write(
        paint(
          'dim',
          "\n  Cursor will need the *ngrok* URL (not localhost), and the generated bearer\n" +
            '  token as the API key. Both are printed in the startup card below — and\n' +
            '  copied to your clipboard automatically once the tunnel is up.\n',
        ),
      );
    } else {
...
export function printStartupCard({ port, host, authPath, model, version, publicUrl, authToken }) {
  ...
  const lines = [
    ...
    paint(
      'dim',
      `listening on ${localUrl}  ·  auth: ${shortenHome(authPath)}  ·  upstream: ${model}` +
