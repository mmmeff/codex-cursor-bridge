...
import { whichNgrok, ngrokAuthtokenPresent } from './tunnel.mjs';
...
async function offerCursorConfig(rl, baseUrl) {
  ...
  stdout.write(
    paint(
      'dim',
      "  Note: Cursor stores BYOK settings in an encrypted SQLite store, not a\n" +
        "  config file, so this can't be fully scripted. The wizard can copy the\n" +
        "  base URL to your clipboard and bring Cursor to the foreground — you'll\n" +
        "  still paste it yourself in the settings dialog.\n",
    ),
  );
  const yes = await promptYesNo(
    rl,
    'Copy the proxy URL to your clipboard and open Cursor now?',
    true,
  );
...
export async function runSetupWizard({ port, host, authPath, bridgeModels = [], bridgeModelMap, version, repoDir }) {
  ...
  let tunnelChoice = false;
  try {
    // Step 2: tunnel for Cursor BYOK
    stdout.write(paint('bold', '\n[2/3] Cursor BYOK requires a public tunnel.\n'));
    tunnelChoice = await offerTunnel(rl);

    // Step 3: daemon install
    stdout.write(paint('bold', '\n[3/3] Run on every login?\n'));
    await offerDaemonInstall(rl, repoDir);

    if (tunnelChoice) {
      const summary = bridgeModelMap
        ? Object.entries(bridgeModelMap)
            .map(([alias, v]) => `${alias} → ${v.upstream}`)
            .join(', ')
        : bridgeModels.join(', ');
      stdout.write(
        paint(
          'dim',
          '\n  When the tunnel comes up the bridge will also patch your Cursor\n' +
            "  state DB to register the bridge-* model aliases below and point\n" +
            "  the OpenAI base URL at the new ngrok URL. You'll still need to\n" +
            '  paste the bearer token into the API-Key field manually (Cursor\n' +
            '  stores keys in an OS-level secret store we can\'t write).\n' +
            (summary ? `  Aliases that will be registered:\n    ${summary}\n` : ''),
        ),
      );
    } else {
...
async function offerTunnel(rl) {
  stdout.write(
    paint(
      'dim',
      "  Cursor's BYOK feature actually runs requests through Cursor's cloud,\n" +
        '  not from your editor — so a plain localhost URL is unreachable.\n' +
        '  An ngrok tunnel exposes the bridge through a public https URL that\n' +
        "  Cursor's backend can reach. The bridge auto-generates a bearer\n" +
        '  token you paste into the API-Key field, so the public URL alone\n' +
        '  cannot spend your quota.\n',
    ),
  );

  if (!whichNgrok()) {
    stdout.write(paint('yellow', '  ! ngrok is not installed on this machine.\n'));
    stdout.write(
      paint(
        'dim',
        '    Install with `brew install --cask ngrok` (macOS) or download from\n' +
          '    https://ngrok.com/download. Then re-run `codex-cursor-bridge --setup`.\n',
      ),
    );
    return false;
  }
  if (!ngrokAuthtokenPresent()) {
    stdout.write(paint('yellow', '  ! ngrok is installed but has no authtoken configured.\n'));
    stdout.write(
      paint(
        'dim',
        '    Sign up (free) at https://dashboard.ngrok.com/get-started/your-authtoken,\n' +
          '    then run: ngrok config add-authtoken <YOUR_TOKEN>\n' +
          '    Then re-run `codex-cursor-bridge --setup`.\n',
      ),
    );
    return false;
  }

  const yes = await promptYesNo(rl, 'Enable the ngrok tunnel for Cursor BYOK?', true);
  if (!yes) {
    stdout.write(
      paint(
        'dim',
        "  Skipped. The bridge will run on localhost only, which works for Aider,\n" +
          '  Continue, Cline, the OpenAI SDK, etc., but Cursor BYOK will still call\n' +
          "  Cursor's cloud instead of this proxy.\n",
      ),
    );
    return false;
  }
  stdout.write(paint('green', '  ✓ Tunnel mode will be enabled on next start.\n'));
  return true;
}

// ---------- startup card ----------

export function printStartupCard({
  port,
  host,
  authPath,
  bridgeModelMap = {},
  ...
  const localUrl = `http://${host}:${port}/v1`;
  const exposedBase = publicUrl ? `${publicUrl}/v1` : localUrl;
  const apiKey = authToken ?? 'sk-bridge';
  const apiKeyHint = authToken ? '' : ' (any non-empty value works)';
  const entries = Object.entries(bridgeModelMap);
  const aliasW = Math.max(0, ...entries.map(([k]) => k.length));
  const upstreamW = Math.max(0, ...entries.map(([, v]) => v.upstream.length));
  const modelLines = entries.length
    ? entries.map(
        ([alias, { upstream, tagline }]) =>
          `       ${paint('bold', alias.padEnd(aliasW))}  →  ${paint('bold', upstream.padEnd(upstreamW))}  ${paint('dim', tagline ?? '')}`,
      )
    : [paint('dim', '       (no bridge models configured)')];
  const lines = [
    paint('bold', `codex-cursor-bridge v${version}`),
    paint(
      'dim',
      `listening on ${localUrl}  ·  auth: ${shortenHome(authPath)}` +
        (publicUrl ? `\npublic tunnel: ${publicUrl} (ngrok)` : ''),
    ),
    '',
    paint('cyan', '── Cursor BYOK setup ' + '─'.repeat(Math.max(0, 50))),
    `  1. ${paint('bold', 'Cmd+,')} (or Ctrl+,) → ${paint('bold', 'Cursor Settings → Models')}`,
    `  2. "OpenAI API Key":  ${paint('bold', apiKey)}${paint('dim', apiKeyHint)}`,
    `  3. Toggle ${paint('bold', '"Override OpenAI Base URL"')} on`,
    `  4. Base URL:  ${paint('bold', exposedBase)}`,
    `  5. In the model picker pick one of (alias → real model):`,
    ...modelLines,
    paint('dim', '       (opaque aliases dodge Cursor\'s premium-name matcher; the bridge'),
    paint('dim', '        auto-adds them under "Add Model" when Cursor is fully quit)'),
  ...
  if (publicUrl) {
    lines.splice(
      ...
      paint(
        'yellow',
        '  ! Anyone with this ngrok URL who also knows your API key above can\n' +
          "    spend your ChatGPT quota. Don't share the URL+token pair.\n",
      ),
    );
  }
