...
async function offerDaemonInstall(rl, repoDir) {
  if (process.platform === 'darwin') {
    const yes = await promptYesNo(rl, 'Install as a macOS LaunchAgent so it auto-starts on login?', true);
    ...
    try {
      ...
      stdout.write(
        paint('dim', '    (when running via `npx`, only the published files are present — install scripts ship in the package)\n'),
      );
