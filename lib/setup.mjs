...
export async function runSetupWizard({ port, host, authPath, bridgeModels = [], version, repoDir }) {
      ...
      stdout.write(
        paint(
          ...
          '\n  When the tunnel comes up the bridge will also patch your Cursor\n' +
            ...
            (bridgeModels.length ? `  Models that will be registered: ${bridgeModels.join(', ')}\n` : ''),
        ),
      );
    } else {
...
export function printStartupCard({
  ...
  const modelList = bridgeModels.length ? bridgeModels.join(', ') : '(none configured)';
  const lines = [
    ...
    `  5. In the model picker pick one of:  ${paint('bold', modelList)}`,
    paint('dim', '       (the bridge auto-adds these to Cursor under "Add Model"; pick one to use)'),
